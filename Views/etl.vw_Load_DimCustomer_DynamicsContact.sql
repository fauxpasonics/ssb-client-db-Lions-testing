...
CREATE VIEW [etl].[vw_Load_DimCustomer_DynamicsContact] AS 
...
            , z.parentcustomeridname AS [CompanyName] --UPDATED
...
            , parentcustomerid AS[ExtAttribute7] --nvarchar(100)
...
            , 0 AS IsBusiness
			...
			, CASE WHEN z.new_SSBCRMSYSTEMSSIDWinner IS NULL OR z.new_SSBCRMSystemSSIDWinnerSourceSystem IS NULL THEN NULL
			   ...
			   WHEN z.new_SSBCRMSystemSSIDWinnerSourceSystem = 'TM' THEN 'TM-' + z.new_SSBCRMSYSTEMSSIDWinner
...
        FROM lions_reporting.prodcopy.contact z WITH (NOLOCK) 


        WHERE 1=1
        AND DATEDIFF(DAY,z.modifiedon,GETDATE())<=7 OR DATEDIFF(DAY,z.copyloaddate,GETDATE())<=7 --This needs to be uncommented out after the first load, update the date fields
    ) a
 
)
