...
CREATE VIEW [ods].[vw_LoadDimCustomer_LiveA] AS ( --Change "SOURCESYSTEM" to the correct source
...
--base set
		SELECT
			...
			, 'LiveA' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			, NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules

			/*Standard Attributes*/
			, CAST(z.la_id AS NVARCHAR(200)) [SSID] -- Required, must be cast as NVARCHAR(200)
			...
			/*Name*/
			, NULL AS [Prefix]
			, z.cust_first_nm AS [FirstName]
			, z.cust_middle_nm AS [MiddleName]
			, z.cust_last_nm AS [LastName]
			, NULL AS [Suffix]
			, NULL AS [Fullname]
			--, c.name_title as [Title]

			/*AddressPrimary*/
            , CASE WHEN ISNULL(z.cust_addr_line_2,'')='' THEN z.cust_addr_line_1 ELSE z.cust_addr_line_1+' '+z.cust_addr_line_2 END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street eg. CASE WHEN ISNULL(address2,'') <> '' THEN z.street + ' ' + z.address2 ELSE z.street END AS [AddressPrimaryStreet]
			, NULL AS [AddressPrimarySuite] -- ALWAYS LEAVE THIS NULL!!!!!!!!!!!!!!!!
			, z.cust_city_nm AS [AddressPrimaryCity] 
			, z.cust_state_nm	AS [AddressPrimaryState] 
			, z.cust_postal_cd AS [AddressPrimaryZip] 
			, NULL AS [AddressPrimaryCounty]
			, z.cust_ctry_nm AS [AddressPrimaryCountry] 
			...
			/*Phone*/
			, LEFT(z.cust_phn_num_1,25) AS [PhonePrimary] -- For Coalescing values make sute they are wrapped in a NULLIF for values that = '' eg COALESCE(NULLIF(z.mobile,''),z.phone) AS [PhonePrimary]
			, LEFT(z.cust_phn_num_2,25) AS [PhoneHome]
			...
			/*Email*/
			, CAST(z.cust_email_addr AS NVARCHAR(256)) AS [EmailPrimary]
		...
		--SELECT COUNT(*)			
		FROM [ods].LiveAnalytics_Customer z WITH (NOLOCK) --Change To Source Table
		WHERE 1=1
