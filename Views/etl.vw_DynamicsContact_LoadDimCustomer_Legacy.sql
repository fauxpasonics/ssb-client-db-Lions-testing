...
CREATE VIEW [etl].[vw_DynamicsContact_LoadDimCustomer_Legacy] AS ( --Change "Source" to the correct source
 
    SELECT *
    /*Name*/
    , HASHBYTES('sha2_256',
...
            , 'Legacy_Dynamics_Contact' AS [SourceSystem] --Put source name such as TM or CRM_Contact
...
            , CAST(NULL AS NVARCHAR (50)) AS [CustomerType] --new_accounttypedescription? Individual vs. Staff
            , CAST(NULL AS NVARCHAR (50)) AS [CustomerStatus]
            , CAST(NULL AS NVARCHAR (50)) AS [AccountType] 
            , CAST(ownerid AS NVARCHAR (50)) AS [AccountRep] --new_accountrepresentative? UPDATE?
            , NULL AS [CompanyName] --UPDATED
...
            , z.suffix AS [Suffix]
			, CASE WHEN z.suffix IS NULL THEN z.fullname ELSE z.fullname + ' ' + z.suffix END AS [Fullname]
...
            , LEFT(MobilePhone,25) AS [PhoneCell] --UPDATED
            , LEFT(telephone1,25) AS [PhoneBusiness]
            , LEFT(NULL,25) AS [PhoneFax]
            , LEFT(NULL,25) AS [PhoneOther]
 
            /*Email*/
            , CAST(z.emailaddress1 AS NVARCHAR(256)) AS [EmailPrimary]
            , CAST(z.emailaddress2 AS NVARCHAR(256)) AS [EmailOne]
            , CAST(z.emailaddress3 AS NVARCHAR(256)) AS [EmailTwo]
 
            /*Extended Attributes*/
            , CAST(lions_veritixaccountnumber AS NVARCHAR(100)) AS[ExtAttribute1] --nvarchar(100)
...
            , lions_veritixaccountnumber AS [AccountId] --
            , 0 AS IsBusiness
			, statecode AS IsDeleted
            , CASE WHEN lions_veritixaccountnumber IS NULL THEN NULL 
				ELSE  'VTX-' + lions_veritixaccountnumber
...
        FROM lions_reporting.prodcopy.contact_legacy z WITH (NOLOCK) 


        WHERE 1=1 AND statecode = 0
        AND DATEDIFF(DAY,z.copyloaddate,GETDATE())<=3 --This needs to be uncommented out after the first load, update the date fields
