...
CREATE VIEW [ods].[vw_TEX_LoadDimCustomer] AS ( --Change "Source" to the correct source
...
--base set
		SELECT
			...
			, 'Ticket Exchange' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			, NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules

			/*Standard Attributes*/
			, CAST(z.ETL__ID AS NVARCHAR(200)) [SSID] -- Required
			...
			/*Name*/
			, NULL AS [Prefix]
			, z.buyerfirstname AS [FirstName]
			, NULL AS [MiddleName]
			, z.buyerlastname AS [LastName]
			, NULL AS [Suffix]
			, NULL AS [Fullname]
			--, c.name_title as [Title]

			/*AddressPrimary*/
            , CONCAT(z.addressline1,' ',z.addressline2) AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
			...
			/*Phone*/
			, LEFT(z.dayphone,25) AS [PhonePrimary]
			, LEFT(z.eveningphone,25) AS [PhoneHome]
			...
			, z.ETL__CreatedDate [SSCreatedDate] --createddate
			...
			, 0 AS IsBusiness
			, 0 AS [IsDeleted]
			, 'TE-' + CAST(z.ETL__ID AS NVARCHAR(100)) AS [Customer_Matchkey] /*Standard for macthkeys are ‘TM-xxxx’, ‘Pac-xxxx’ depending on source, 
											Customer Matchkey needs to have fairly complicated logic. 
											See Writup for details.*/
		--SELECT COUNT(*)			
		FROM [ods].TEX_DerivedCustomers z WITH (NOLOCK) --Change To Source Table
		WHERE 1=1
		--AND DATEDIFF(DAY,z.lastModified,GETDATE())<=10 OR DATEDIFF(DAY,z.added,GETDATE())<=10 --This needs to be uncommented out after the first load, update the date fields
	) a
