...
CREATE VIEW [ods].[vw_TM_Historical_LoadDimCustomer] AS ( --Change "Source" to the correct source
...
--base set
		SELECT
			...
			, 'TM_Historical' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			, NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules

			/*Standard Attributes*/
			, CAST(CONCAT(z.acct_id,':',z.cust_name_id) AS NVARCHAR(100)) [SSID] -- Required
			...
			/*Name*/
			, NULL AS [Prefix]
			, z.name_first AS [FirstName]
			, NULL AS [MiddleName]
			, z.name AS [LastName]
			, NULL AS [Suffix]
			, z.full_name_1 AS [Fullname]
			--, c.name_title as [Title]

			/*AddressPrimary*/
            , z.street_addr_1+' '+z.street_addr_2 AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
			, z.city AS [AddressPrimaryCity] 
			, z.state	AS [AddressPrimaryState] 
			, z.zip AS [AddressPrimaryZip] 
			...
			/*Phone*/
			, LEFT(z.phone_day,25) AS [PhonePrimary]
			, LEFT(z.phone_eve,25) AS [PhoneHome]
			, LEFT(z.phone_cell,25) AS [PhoneCell]
			, LEFT(NULL,25) AS [PhoneBusiness]
			, LEFT(z.phone_fax,25) AS [PhoneFax]
			...
			, z.add_date [SSCreatedDate] --createddate
			, z.upd_datetime [SSUpdatedDate] --lastmodifiedbyid
...
--		select top 100 *		
		FROM stg.TM_customer z WITH (NOLOCK) --Change To Source Table
		WHERE 1=1
		AND (z.add_date > DATEADD(DAY,-5,GETDATE()) OR z.upd_datetime > DATEADD(DAY,-5,GETDATE()))	--This needs to be uncommented out after the first load, update the date fields
	) a
