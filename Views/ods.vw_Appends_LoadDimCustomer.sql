...
CREATE VIEW [ods].[vw_Appends_LoadDimCustomer] AS ( --Change "Source" to the correct source
...
--base set
		SELECT
			...
			, 'Appends' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			, NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules

			/*Standard Attributes*/
			, CAST(ETL__SSID AS NVARCHAR(100)) [SSID] -- Required
			...
			/*Name*/
			, NULL AS [Prefix]
			, NULLIF(cvFirstName1	 ,'') AS [FirstName]
			, NULLIF(cvMiddleName1 ,'') AS [MiddleName]
			, NULLIF(cvLastName1	 ,'') AS [LastName]
			, NULLIF(cvSuffixTitle1,'') AS [Suffix]
			, NULL AS [Fullname]
			--, c.name_title as [Title]

			/*AddressPrimary*/
            , [Address] +' '+ [Address 2]	AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
			, [City]						AS [AddressPrimaryCity] 
			, [State]						AS [AddressPrimaryState] 
			, [ZIP Code]					AS [AddressPrimaryZip] 
			...
			, BatchDate AS createddate 
			, BatchDate AS updateddate
			, NULL [AccountId]
			, 0 AS IsBusiness
			, 0 AS IsDeleted
			, NULL AS [Customer_Matchkey] /*Joining the "Source" and corresponding "Accounts" fields, no reliable matches were found to dimcustomer*/
--		select top 100 *		
		FROM ods.Appends
