...
SELECT ssbid.SSB_CRMSYSTEM_CONTACT_ID,
       ...
       DimPromo.OfferCode AS Offer_Code,
       DimPromo.OfferName AS Offer_Name,
...
--, fts.SSCreatedBy Archtics_AddUser --Only for TM clients
FROM dbo.FactTicketSales fts WITH (NOLOCK) --4867678
    INNER JOIN dbo.DimPriceCode DimPriceCode WITH (NOLOCK) --4863342
        ON DimPriceCode.DimPriceCodeId = fts.DimPriceCodeId
    INNER JOIN dbo.DimTicketType DimTicketType WITH (NOLOCK)--4720542
        ON DimTicketType.DimTicketTypeId = fts.DimTicketTypeId
    INNER JOIN dbo.DimPlanType DimPlanType WITH (NOLOCK) --4313679
        ON DimPlanType.DimPlanTypeId = fts.DimPlanTypeId
    LEFT OUTER JOIN dbo.DimSeatType DimSeatType WITH (NOLOCK)--4313679
        ON DimSeatType.DimSeatTypeId = fts.DimSeatTypeId
    INNER JOIN dbo.DimTicketCustomer DimTicketCustomer WITH (NOLOCK) --4312254
        ON fts.DimTicketCustomerId = DimTicketCustomer.DimTicketCustomerId
    INNER JOIN dbo.DimCustomer DimCustomer WITH (NOLOCK) --4312254
        ON DimCustomer.SSID = LEFT(DimTicketCustomer.ETL__SSID, CHARINDEX('.',DimTicketCustomer.ETL__SSID)-1)
           AND DimCustomer.SourceSystem = 'Veritix'
    LEFT OUTER JOIN dbo.DimCustomer AccountRep WITH (NOLOCK) --4312254
        ON AccountRep.DimCustomerId = fts.DimRepId
    INNER JOIN dbo.DimDate DimDate WITH (NOLOCK)--4312254
        ON DimDate.DimDateId = fts.DimDateId
    INNER JOIN dbo.DimSeason DimSeason WITH (NOLOCK)--4312254
        ON DimSeason.DimSeasonId = fts.DimSeasonId
    INNER JOIN dbo.DimEvent DimEvent WITH (NOLOCK)--4312254
        ON DimEvent.DimEventId = fts.DimEventId
    INNER JOIN dbo.dimcustomerssbid ssbid WITH (NOLOCK)--4312254
        ON ssbid.DimCustomerId = DimCustomer.DimCustomerId
    LEFT OUTER JOIN dbo.DimSalesCode DimSaleCode WITH (NOLOCK)--4312254
        ON DimSaleCode.DimSalesCodeId = fts.DimSalesCodeId
    INNER JOIN dbo.DimOffer DimPromo WITH (NOLOCK) --4312254
        ON DimPromo.DimOfferId = fts.DimOfferId
    INNER JOIN dbo.DimSeat DimSeat WITH (NOLOCK)--3930511
        ON DimSeat.DimSeatId = fts.DimSeatId_Start 
    INNER JOIN dbo.DimItem DimItem WITH (NOLOCK)--3930511
        ON DimItem.DimItemId = fts.DimItemId
    LEFT OUTER JOIN dbo.DimEventHeader DimEventHeader WITH (NOLOCK) --3930511
        ON DimEventHeader.DimEventHeaderId = DimEvent.DimEventHeaderID
    LEFT OUTER JOIN dbo.DimSeasonHeader DimSeasonHeader WITH (NOLOCK)
        ON DimSeasonHeader.DimSeasonHeaderId = DimEventHeader.DimSeasonHeaderId
    LEFT OUTER JOIN dbo.DimTeam DimTeam WITH (NOLOCK)
        ON DimEventHeader.OpponentDimTeamId = DimTeam.DimTeamId
    INNER JOIN dbo.DimPlan DimPlan WITH (NOLOCK)
