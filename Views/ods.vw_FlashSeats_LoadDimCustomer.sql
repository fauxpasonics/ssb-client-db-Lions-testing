...
CREATE VIEW [ods].[vw_FlashSeats_LoadDimCustomer] AS ( --Change "Source" to the correct source
...
--base set
		SELECT
			...
			, 'FlashSeats' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			, NULL AS [SourceSystemPriority] --Depends on if client uses sourcesystem/sourcesystempriority tables for MDM Rules

			/*Standard Attributes*/
			, CAST(a.account_id AS NVARCHAR(200)) [SSID] -- Required
			...
			/*AddressPrimary*/
            , CASE WHEN ISNULL(ad.address_line_2,'')<>'' THEN ISNULL(ad.address_line_1,'')+' '+ad.address_line_2 ELSE ISNULL(ad.address_line_1,'') END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
			, ISNULL(ad.city,'') AS [AddressPrimaryCity] 
			, ISNULL(ad.state,'') AS [AddressPrimaryState] 
			, ISNULL(ad.zip,'') AS [AddressPrimaryZip] 
			, NULL AS [AddressPrimaryCounty]
			, ISNULL(ad.country,'')	AS [AddressPrimaryCountry] 
			...
			/*Phone*/
			, LEFT(a.home_phone_number,25) AS [PhonePrimary]
			, LEFT(a.home_phone_number,25) AS [PhoneHome]
			, LEFT(a.mobile_phone_number,25) AS [PhoneCell]
			, LEFT(a.work_phone_number,25) AS [PhoneBusiness]
			, LEFT(NULL,25) AS [PhoneFax]
			, LEFT(NULL,25) AS [PhoneOther]

			/*Email*/
			, CAST(e.email AS NVARCHAR(256)) AS [EmailPrimary]
			...
			, a.account_created_datetime [SSCreatedDate] --createddate
			, a.update_datetime [SSUpdatedDate] --lastmodifiedbyid
			, GETDATE () AS createddate 
			, GETDATE () AS updateddate
			, NULL [AccountId]
			, 0 AS IsBusiness
			, 0 AS IsDeleted --Update if there is logic for records that are deleted from the source, otherwise leave as 0
			, 'FS-' + CAST(a.account_id AS NVARCHAR(100)) AS [Customer_Matchkey] /*Standard for macthkeys are ‘TM-xxxx’, ‘Pac-xxxx’ depending on source, 
											Customer Matchkey needs to have fairly complicated logic. 
											See Writup for details.*/
		--SELECT COUNT(*)			
		FROM [ods].FS_accounts a WITH (NOLOCK)
		LEFT JOIN ods.FS_address ad ON ad.account_id = a.account_id
		LEFT JOIN ods.FS_emails e ON e.account_id = a.account_id--Change To Source Table
		WHERE 1=1
		AND DATEDIFF(DAY,a.update_datetime,GETDATE())<=10 OR DATEDIFF(DAY,a.account_created_datetime,GETDATE())<=10 --This needs to be uncommented out after the first load, update the date fields
	) a
