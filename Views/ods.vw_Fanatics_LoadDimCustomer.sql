...
CREATE VIEW [ods].[vw_Fanatics_LoadDimCustomer] AS ( --Change "Source" to the correct source
...
--base set
		SELECT
			...
			/*AddressPrimary*/
            , CASE WHEN ISNULL(cust.ShipAddress2,'')<>'' THEN cust.ShipAddress1+' '+cust.ShipAddress2 WHEN ISNULL(cust.ShipAddress2,'')='' THEN cust.ShipAddress1 END AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
			, cust.ShipAddressCity AS [AddressPrimaryCity] 
			, cust.ShipAddressState	AS [AddressPrimaryState] 
			, cust.ShipAddressZip AS [AddressPrimaryZip] 
			, NULL AS [AddressPrimaryCounty]
			, cust.ShipAddressCounty AS [AddressPrimaryCountry] 
			
			, CASE WHEN ISNULL(cust.BillAddress2,'')<>'' THEN cust.BillAddress1+' '+cust.BillAddress2 WHEN ISNULL(cust.BillAddress2,'')='' THEN cust.BillAddress1 END AS [AddressOneStreet]
			, cust.BillAddressCity AS [AddressOneCity] 
			, cust.BillAddressState AS [AddressOneState] 
			, cust.BillAddressZip AS [AddressOneZip] 
			, NULL AS [AddressOneCounty] 
			, cust.BilAddressCountry AS [AddressOneCountry] 
