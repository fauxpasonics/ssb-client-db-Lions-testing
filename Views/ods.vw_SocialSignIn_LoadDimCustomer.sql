...
CREATE VIEW [ods].[vw_SocialSignIn_LoadDimCustomer] AS ( --Change "Source" to the correct source
...
--base set
		SELECT
			...
			, 'SocialSignIn' AS [SourceSystem] --Put source name such as TM or CRM_Contact
			...
			/*AddressPrimary*/
            , NULL AS [AddressPrimaryStreet] -- dont load suites combine addresses with multiple lines and load to street
			...
			/*Phone*/
			, LEFT(z.phone,25) AS [PhonePrimary]
			...
			, z.created [SSCreatedDate] --createddate
			...
			, 0 AS IsDeleted
		--SELECT COUNT(*)			
		FROM ods.SSO_CustomerComposite z WITH (NOLOCK) 
		WHERE 1=1
		AND DATEDIFF(DAY,z.created,GETDATE())<=10 --This needs to be uncommented out after the first load, update the date fields
	) a
